...
class ApiClient {
  ...
  Future<http.Response> post(String path, {Map<String, String>? headers, Object? body, int timeoutSeconds = 8}) {
    return _send('POST', path, headers: headers, body: body, timeoutSeconds: timeoutSeconds);
  }

  Future<http.Response> get(String path, {Map<String, String>? headers, int timeoutSeconds = 8}) {
    return _send('GET', path, headers: headers, timeoutSeconds: timeoutSeconds);
  }

  Future<http.Response> _send(String method, String path, {Map<String, String>? headers, Object? body, int timeoutSeconds = 8}) async {
    final uri = Uri.parse('$baseUrl$path');
    final authToken = AuthService().token;
    // Merge headers; allow caller to request skipping automatic Authorization
    final skipAuth = headers != null && (headers['X-Skip-Auth'] == '1' || headers['X-Skip-Auth'] == 'true');
    final h = <String, String>{'Content-Type': 'application/json', if (headers != null) ...headers};
